  ...
  return (
    ...
    >
      <MaxWidthWrapper className="flex items-center justify-between w-full h-[70px]">
        ...
        {/* Desktop Navigation */}
        <div className="hidden md:flex w-full flex-grow items-center justify-end gap-x-12">
          <div className="w-fit h-[70px] flex items-center justify-end gap-x-8">
            {navs.map((item) => (
              <button
                key={item}
          ...
          <div className="w-fit flex items-center justify-end gap-x-2">
            <Link
              href={"/en/sign-up"}
              className="px-4 py-2  font-extrabold rounded-full bg-white border border-[#f46506] text-[#f46506] hover:bg-gray-100 transition-colors"
            >
              login
            </Link>
